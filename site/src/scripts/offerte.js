...
(function () {
	...
	function initWizard( root ) {
		...
		function validateStep() {
			...
			var choices = panel.querySelectorAll( 'input[type="radio"], input[type="checkbox"]:not([name="consent"])' );
			if ( choices.length ) {
		...
		function initAreaTool() {
			...
			function recalc() {
				...
				if ( total > 0 ) {
					form.querySelectorAll( 'input[name="oppervlakte"]:checked' ).forEach( function ( input ) {
						input.checked = false;
					} );
				}
			...
			form.querySelectorAll( 'input[name="oppervlakte"]' ).forEach( function ( input ) {
		...
		function answers( key ) {
			var sel = 'input[name="' + key + '"]:checked, input[name="' + key + '[]"]:checked';
		...
		function buildSummary() {
			...
			panels.forEach( function ( panel ) {
				...
				panel.querySelectorAll( 'input:not([type="checkbox"]):not([type="radio"]), textarea' ).forEach( function ( input ) {
					if ( input.name === 'website' || ! input.value.trim() || input.closest( '[data-area]' ) ) {
						return;
					}
			...
			window.fetch( config.endpoint, {
				...
				.then( function ( response ) {
					return response.json().catch( function () {
						return { ok: false, message: i18n.genericErr };
					} );
				} )
				.then( function ( result ) {
					...
					showError( ( result && result.message ) || i18n.genericErr );
				} )
				.catch( function () {
