/** Inline SVG-iconen; dezelfde set als in het oude thema. */
const paths: Record<string, string> = {
  ...
  google: '<path d="M21.35 11.1H12v3.2h5.35a4.6 4.6 0 0 1-2 3.02v2.5h3.23c1.9-1.74 2.99-4.3 2.99-7.35 0-.7-.07-1.4-.22-2.07z"/><path d="M12 22c2.7 0 4.96-.9 6.61-2.43l-3.23-2.5c-.9.6-2.05.95-3.38.95-2.6 0-4.8-1.75-5.59-4.1H3.08v2.58A10 10 0 0 0 12 22z"/><path d="M6.41 13.92a6 6 0 0 1 0-3.84V7.5H3.08a10 10 0 0 0 0 9z"/><path d="M12 5.98c1.47 0 2.79.5 3.83 1.5l2.86-2.86A9.6 9.6 0 0 0 12 2a10 10 0 0 0-8.92 5.5l3.33 2.58C7.2 7.73 9.4 5.98 12 5.98z"/>',
...
/** Gevulde iconen krijgen geen contourlijn; die maakt de vorm modderig. */
...
export function icon(name: string, size = 24): string {
  ...
  return `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="${isFilled ? 'currentColor' : 'none'}" stroke="${isFilled ? 'none' : 'currentColor'}" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false">${d}</svg>`;
}

export function stars(count = 5): string {
  return `<span class="ifs-stars" role="img" aria-label="${count} van de 5 sterren">${icon('star', 17).repeat(count)}</span>`;
}
